...
class AyStar
{
	_queue_class = import("queue.fibonacci_heap", "", 2);
	_pf_instance = null;
	_cost_callback = null;
...
function AyStar::InitializePath(sources, goals, ignored_tiles = [])
{
	...
	foreach (node in sources) {
		if (typeof(node) == "array") {
			...
			this._open.Insert(new_path, new_path._cost + this._estimate_callback(this._pf_instance, new_path._tile, new_path._direction, new_path._params, goals));
		} else {
			this._open.Insert(node, node._cost + this._estimate_callback(this._pf_instance, node._tile, node._direction, node._params, goals));
		}
	}
...
function AyStar::FindPath(iterations)
{
	...
	while (this._open.Count() > 0 && (iterations == -1 || iterations-- > 0)) {
		...
		if (this._closed.HasItem(cur_tile)) {
			...
			if ((this._closed.GetValue(cur_tile) & path._direction) != 0) continue;
			...
			while (scan_path != null) {
				if (scan_path._tile == cur_tile) {
					if (!this._check_direction_callback(this._pf_instance, cur_tile, scan_path._direction, path._direction, path._params)) {
						mismatch = true;
						break;
					}
				}
			...
			this._closed.SetValue(cur_tile, this._closed.GetValue(cur_tile) | path._direction);
		} else {
			/* New entry, make sure we don't check it again */
			this._closed.AddItem(cur_tile, path._direction);
		}
		...
		foreach (node in neighbours) {
			...
			this._open.Insert(new_path, new_path._cost + this._estimate_callback(this._pf_instance, new_path._tile, new_path._direction, new_path._params, this._goals));
		}
	}
...
class AyStar.Path
{
	...
	constructor(old_path, new_tile, new_direction, new_params, cost_callback, pf_instance)
	{
		this._prev = old_path;
		this._tile = new_tile;
		this._direction = new_direction;
		this._params = new_params;
		this._cost = cost_callback(pf_instance, old_path, new_tile, new_direction, new_params);
		if (old_path == null) {
			this._length = 0;
		} else {
	...
	/**
	 * Return the additional custom parameters that this tile (might) have.
	 */
